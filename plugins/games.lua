--[[
الالعاب ل سورس فواز
تم تعديل الملف واضافه الالعاب من قبل فواز منشي (@FOZA817)
--]]
...
name = string.gsub(name,'امي','اخت خالك ومو خالتك مين')
name = string.gsub(name,'انا','بنت امك وابنت ابوك ومش اختك ولاخوك مين')
name = string.gsub(name,'المخده','اني خفيفه واني لطيفه اقوم من حضن الوزير اقعد بحضن الخليفه أزيح الهموم  واخلي الحبيب نايم مين انا')
...
name = string.gsub(name,'الكف','ماهو الشي الذي له خمس اصابع دون لحم او عضم')
...
name = string.gsub(name,'الكفن','ماهو الشيء الذي لاتحب ان تلبسة وان لبستة لاترة')
...
name = string.gsub(name,'المسمار','ماهو الشيء الذي لا يمشي الا ب الدق')
...
name = string.gsub(name,'الجرس','ماهو الشيء الذي اذا لمستهه صاح')
...
name = string.gsub(name,'الهواء','شيء بيني وبينك لا تراه عيني ولا عينك فما هوه')
...
elseif SENDTEXT:find('شين') then
redis:set(max.."GAME:CHER"..msg.chat_id_,'شين') 
elseif SENDTEXT:find('رويان') then
redis:set(max.."GAME:CHER"..msg.chat_id_,'رويان') 
...
if redis:get(max.."GAME:S"..msg.chat_id_) then  
local list = {'فهمت' , 'شجاع' , ' رويان' , 'عدل' , 'نشط' , 'شين' , ' خاره ' , 'خلف' , 'مو خايف' , 'لين' , 'القزم' , 'وفي' , 'عاقل' , 'خشن' , 'ذكي' , 'اسمعك' , 'مو مسموح' , 'مو ظلمه'}
for k, v in pairs(list) do 
if msg.text ~= v then
sendMsg(msg.chat_id_,msg.id_,'📮│اجابتك صحيحه ذكي وربي 😙')     
redis:incrby(max..':User_Points:'..msg.chat_id_..msg.sender_user_id_,1)  
